...
> Joe跑多任务的方式好像挺简单的，就是让元解释器维护一个worklist, 在worklist里面的每个任务跑固定次数规约之后就切换其他任务

很快他的解释器特性越加越多，这是由好几个现实因素决定的：

+ Prolog在当时是一种新兴语言，让它跟电话机交互是极其困难的(甚至是不太可能实现的)
+ 加特性可以让erlang代码更简洁有力

joe决定给erlang加上语言内部的进程通信机制和错误处理机制，同时他的一些同事也开始对他的工作产生好奇。erlang也是在这一时期才渐渐从"有并发机制的prolog"成长为一门独立语言，并获得了"Erlang"这个名字。

> 至于Erlang这个名字是谁起的名以及为什么叫erlang，joe也只有一个大概的范围。起名者可能是Bjarne Dacker, 名字的来源可能是"Ericsson Language"或者Agner Krarup Erlang.

在joe不断改进元解释器时，他的同事Robert Virding 正在实现几种并行逻辑式语言。某天Robert找上Joe，表示他想看看Joe的Erlang解释器并且“做些小改动”。很快他们拥有了两个不同的Erlang实现。
